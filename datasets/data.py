...
def get_data(args):
    if args.dataset in ['cora', 'polblogs', 'Homo_sapiens']:
        ...
        max_degree = max([max([d for n, d in train_nx_graph.degree()]) for train_nx_graph in train_nx_graphs])
        # # 加载GAE模型
        model = torch.load("gae/weight/elliptic_gae_relu_256.pt")
        # 增加子图的节点特征属性
        ...
        max_degree = max([max([d for n, d in train_nx_graph.degree()]) for train_nx_graph in train_nx_graphs])
        # # 加载GAE模型
        model = torch.load("gae/weight/dgraph_gae_relu_32.pt")
        # model=None
        ...
        max_degree = max([max([d for n, d in train_nx_graph.degree()]) for train_nx_graph in train_nx_graphs])
        # 加载GAE模型
        model = torch.load("gae/weight/reddit_gae_relu_128.pt")
        # 增加子图的节点特征属性
        featureNameList=["label","feature"]
        for i in range(64):
            featureNameList.append(f"feature{i}")
        ...
        max_degree = max([max([d for n, d in train_nx_graph.degree()]) for train_nx_graph in train_nx_graphs])
        # 加载GAE模型
        model = torch.load("gae/weight/photo_gae_relu_896.pt")
        # 增加子图的节点特征属性
        featureNameList=["label","feature"]
        for i in range(745):
            featureNameList.append(f"feature{i}")
        ...
        max_degree = max([max([d for n, d in train_nx_graph.degree()]) for train_nx_graph in train_nx_graphs])
        # 加载GAE模型
        model = torch.load("gae/weight/tfinance_gae_sigmoid_32.pt")
        # 增加子图的节点特征属性
        featureNameList=["label","feature"]
        for i in range(10):
            featureNameList.append(f"feature{i}")
